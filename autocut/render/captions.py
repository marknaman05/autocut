...
_FALLBACK_FONTS = ("Arial Black.ttf", "Arial Bold.ttf", "Helvetica.ttc", "Arial.ttf")
...
def find_font(name: str) -> Path:
    """Resolve a font family name to a file on disk."""
    candidates = [f"{name}.ttf", f"{name}.ttc", f"{name.replace(' ', '')}.ttf", *_FALLBACK_FONTS]
...
class _Renderer:
    ...
    def __init__(self, config: CaptionConfig, width: int) -> None:
        ...
        self.height = int(
            config.font_size * config.pop_scale * 1.6 + config.outline * 2 + config.shadow_offset
        )
    ...
    def _widths(self, line: Line, popped_index: int | None = None) -> list[float]:
        ...
        """
        return [
            (self.pop_font if index == popped_index else self.font).getlength(word.text)
    ...
    def render(self, line: Line, active: int, popped: bool) -> Image.Image:
        ...
        widths = self._widths(line, active if popped else None)
        ...
        for index, (word, width) in enumerate(zip(line.words, widths)):
            ...
            colour = config.highlight_colour if is_active else config.text_colour
            ...
            if config.shadow_offset:
                draw.text(
                    ...
                    word.text, font=font, fill=config.shadow_colour,
                    anchor="mm", stroke_width=config.outline,
                    stroke_fill=config.shadow_colour,
                )
            draw.text(
                (centre, baseline), word.text, font=font, fill=colour, anchor="mm",
...
def build(
    ...
    renderer = _Renderer(config, width)
